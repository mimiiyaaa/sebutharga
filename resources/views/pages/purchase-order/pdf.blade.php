...
<body>
    <table class="header"><tr>
        <td><div class="company">{{ config('purchase_order.issuer_name') }}</div>
            <div class="lines">{{ config('purchase_order.issuer_address') }}</div>
            <div>No. Tel: {{ config('purchase_order.issuer_phone') }} | E-mel: {{ config('purchase_order.issuer_email') }}</div>
        </td>
    ...
    <table class="addresses"><tr>
        ...
        <td>Alamat Penghantaran:<br><strong>{{ config('purchase_order.issuer_name') }}</strong><div class="lines">{{ $order->delivery_address ?: config('purchase_order.issuer_address') }}</div>U/P: {{ $order->attention_delivery ?: config('purchase_order.delivery_attention') }}<br>No. Tel: {{ config('purchase_order.issuer_phone') }}</td>
    </tr></table>
    <table class="items">
    ...
    <table class="signatures"><tr><td><strong>Disediakan Oleh:</strong><div class="signature">{{ $order->prepared_by }}<br>{{ config('purchase_order.prepared_role') }}</div></td><td><strong>Diluluskan Oleh:</strong><div class="signature">{{ $order->approved_by }}<br>{{ config('purchase_order.approved_role') }}</div></td></tr></table>
</body>
</html>
