...
<a href="{{ (request('from') === 'final' && request('via') !== 'detail' ? route('sebut-harga.final') : route('sebut-harga.edit', [$quotation->quotation_id, 'draft' => $draft->quotation_detail_id, 'from' => request('from') === 'final' ? 'final' : 'draft'])) }}" class="rounded-lg border border-gray-300 bg-white px-4 py-3 text-sm text-gray-700 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300">{{ __(request('from') === 'final' && request('via') !== 'detail' ? 'Kembali ke Final Sebut Harga' : 'Kembali ke Maklumat Sebut Harga') }}</a>
