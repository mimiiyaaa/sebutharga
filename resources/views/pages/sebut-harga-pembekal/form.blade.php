...
@section('content')
    ...
    <form method="POST" enctype="multipart/form-data" action="{{ route('sebut-harga-pembekal.save', ['id' => $quotation?->supplier_quotation_id]) }}" class="font-outfit" x-data="{
        ...
        @csrf
        <div class="space-y-6">
            <section class="mx-auto w-full max-w-4xl overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-theme-xs dark:border-gray-800 dark:bg-white/[0.03]">
                ...
                <div class="min-h-[480px] bg-gray-100 p-3 dark:bg-gray-950/40">
                    <div x-cloak x-show="pdfUrl" class="h-[480px] overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700"><iframe :src="pdfUrl" title="{{ __('Preview PDF Sebut Harga Pembekal') }}" class="h-full w-full"></iframe></div>
