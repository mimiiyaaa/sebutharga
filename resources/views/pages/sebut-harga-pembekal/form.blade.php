        ...
        <div class="grid grid-cols-1 items-start gap-6 lg:grid-cols-2">
            ...
            <div class="space-y-5">
                ...
                <x-common.document-card :title="__('Item Sebut Harga')">
                    <div class="space-y-4">
                        <div class="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700"><table class="w-full min-w-[760px]"><thead class="bg-gray-50 dark:bg-gray-800"><tr>@foreach (['Bil.', 'Perihal', 'Qty', 'Unit', 'Harga (RM)', 'Jumlah (RM)', ''] as $heading)<th class="px-3 py-3 text-start text-theme-xs font-semibold text-gray-500 dark:text-gray-400">{{ __($heading) }}</th>@endforeach</tr></thead><tbody><template x-for="(item, index) in items" :key="item.id"><tr class="border-t border-gray-100 dark:border-gray-800"><td class="p-2 text-center text-sm text-gray-500" x-text="index + 1"></td><td class="min-w-48 p-2"><textarea :name="`items[${index}][description]`" x-model="item.description" rows="2" required class="w-full rounded-lg border border-gray-300 px-2.5 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"></textarea></td><td class="w-20 p-2"><input :name="`items[${index}][quantity]`" x-model.number="item.quantity" type="number" min="1" step="1" required class="w-20 rounded-lg border border-gray-300 px-2 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"></td><td class="w-20 p-2"><input :name="`items[${index}][unit]`" x-model="item.unit" required class="w-20 rounded-lg border border-gray-300 px-2 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"></td><td class="w-28 p-2"><input :name="`items[${index}][price]`" x-model.number="item.price" type="number" min="0" step="0.01" required class="w-28 price-input rounded-lg border border-gray-300 px-2 py-2 text-sm dark:border-gray-700 dark:bg-gray-900 dark:text-white/90"></td><td class="whitespace-nowrap p-2 text-end text-sm font-medium" x-text="money(subtotal(item))"></td><td class="p-2"><button type="button" @click="items.splice(index, 1)" :disabled="items.length === 1" class="text-xs font-medium text-error-600 disabled:opacity-30 dark:text-error-400">{{ __('Buang') }}</button></td></tr></template></tbody></table></div>
